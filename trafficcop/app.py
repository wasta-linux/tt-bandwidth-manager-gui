...
class TrafficCop(Gtk.Application):
    ...
    def do_activate(self):
        ...
        for line in output_list:
            try:
                ...
                unit_file_state = match.group()
                continue
            except:
                pass
            try:
                match = re.match(apat, line)
                active_state = match.group()
                continue
            except:
                pass

        unit_file_state = unit_file_state.split('=')[1]
        active_state = active_state.split('=')[1]
        state = True if unit_file_state == 'enabled' else False
        toggle_unit_state.set_state(state)
        state = True if active_state == 'active' else False
        toggle_active.set_state(state)
    ...
    def do_command_line(self, command_line):
        ...
        if 'version' in options:
            proc = subprocess.run(
                ['apt-cache', 'policy', 'tt-bandwidth-manager'],
